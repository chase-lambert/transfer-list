...
* [`src/transfer_list/`](src/transfer_list/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/transfer_list/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `transfer-list` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/transfer_list/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns transfer-list.example
  (:require [transfer-list.config :as config])
